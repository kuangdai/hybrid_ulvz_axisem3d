...
void Mesh::release(Domain &domain) {
    ...
    for (const auto &point: mGLLPoints) {
        point->release(domain, ipnt++);
    }
    MultilevelTimer::end("Release Points", 2);
    
    
    // form (meshTag, solidTag, fluidTag)
    IMatXX meshSolidFluidAll(mGLLPoints.size(), 3);
    for (int i = 0;i<mGLLPoints.size();i++) {
        meshSolidFluidAll(i, 0) = i;
        meshSolidFluidAll(i, 1) = mGLLPoints[i]->mSolidDomainTag;
        meshSolidFluidAll(i, 2) = mGLLPoints[i]->mFluidDomainTag;
    }
    
    domain.mMapMeshSolidFluid = meshSolidFluidAll;
...
void Mesh::measure(DecomposeOption &measured) {
    ...
    // enhance
    MultilevelTimer::begin("Enhance Weights", 2);    
    for (int iloc = 0; iloc < getNumQuads(); iloc++) {
        int quadTag = mQuads[iloc]->getQuadTag();
        double rcen = mQuads[iloc]->computeCenterRadius();
        double tcen = mQuads[iloc]->computeCenterTheta();
        double scen = rcen * sin(tcen);
        double zcen = rcen * cos(tcen);
        for (int i = 0;i<mDDPar->mEnhancedRegion.size(); i++) {
            bool rt = std::get<0>(mDDPar->mEnhancedRegion[i]);
            if (rt) {
                double r0 = std::get<1>(mDDPar->mEnhancedRegion[i]);
                double r1 = std::get<2>(mDDPar->mEnhancedRegion[i]);
                double t0 = std::get<3>(mDDPar->mEnhancedRegion[i]);
                double t1 = std::get<4>(mDDPar->mEnhancedRegion[i]);
                double f = std::get<5>(mDDPar->mEnhancedRegion[i]);
                if (r0 < rcen && rcen < r1 && t0 < tcen && tcen < t1) {
                    measured.mElemWeights[quadTag] *= f;
                }
            } else {
                double s0 = std::get<1>(mDDPar->mEnhancedRegion[i]);
                double s1 = std::get<2>(mDDPar->mEnhancedRegion[i]);
                double z0 = std::get<3>(mDDPar->mEnhancedRegion[i]);
                double z1 = std::get<4>(mDDPar->mEnhancedRegion[i]);
                double f = std::get<5>(mDDPar->mEnhancedRegion[i]);
                if (s0 < scen && scen < s1 && z0 < zcen && zcen < z1) {
                    measured.mElemWeights[quadTag] *= f;
                }
            }
        }
    }
    MultilevelTimer::end("Enhance Weights", 2);    
...
Mesh::DDParameters::DDParameters(const Parameters &par) {
    ...
    // enhance
    if (par.getValue<std::string>("DD_REGIONS_ENHANCE") == "none") {
        return;
    }
    
    int nregion = par.getSize("DD_REGIONS_ENHANCE");
    for (int ireg=0; ireg<nregion; ireg++) {
        std::string mstr = par.getValue<std::string>("DD_REGIONS_ENHANCE", ireg);
        std::vector<std::string> strs = Parameters::splitString(mstr, "$");
        bool rt = (strs[0] == "rt");
        double r0 = boost::lexical_cast<double>(strs[1]);
        double r1 = boost::lexical_cast<double>(strs[2]);
        double t0 = boost::lexical_cast<double>(strs[3]);
        double t1 = boost::lexical_cast<double>(strs[4]);
        double factor = boost::lexical_cast<double>(strs[5]);
        mEnhancedRegion.push_back(std::tuple<bool, double, double, double, double, double>{rt, r0, r1, t0, t1, factor});
    }
}
