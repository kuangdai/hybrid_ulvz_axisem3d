...
Receiver::Receiver(const std::string &name, const std::string &network, 
    ...
    double depth, bool dumpStrain, bool dumpCurl, bool forceFluid, bool forceSolid,
    double srcLat, double srcLon, double srcDep):
mName(name), mNetwork(network), mDepth(depth), 
mDumpStrain(dumpStrain), mDumpCurl(dumpCurl),
mForceFluid(forceFluid), mForceSolid(forceSolid) {
    RDCol3 rtpG, rtpS;
    if (geographic) {
        rtpG(0) = 1.;
...
bool Receiver::locate(const Mesh &mesh, int &elemTag, int &quadTag, bool depthInRef) const {
    ...
    for (int iloc = 0; iloc < mesh.getNumQuads(); iloc++) {
        ...
        if (mForceFluid && !quad->isFluid()) {
            continue;
        }
        if (mForceSolid && quad->isFluid()) {
            continue;
        }
