...
// begin a sub-process
void MultilevelTimer::begin(const std::string &procName) {
    if (isActivated()) {
        mFile << bstring::filled(mCurrentLevel * 4, '.');
...
void MultilevelTimer::message(const std::string &what) {
    if (isActivated()) {
        mFile << bstring::filled(mCurrentLevel * 4, '.');
        mFile << "<MESSG>  " << what << "\n";
    }
...
// end a sub-process
void MultilevelTimer::ended(const std::string &procName) {
    if (isActivated()) {
        mCurrentLevel--;
