...
namespace bstring {
    ...
    // ****** title ******
    std::string boxTitle(const std::string &title, int length, char fill) {
    ...
    // *******************
    std::string boxBaseline(int length, char fill) {
    ...
    // for double with unit
    std::string boxEquals(int indent, int keyWidth,
                          const std::string &key, double value,
                          double unit, const std::string &unitStr,
                          const std::string &equalSign,
                          int nspaces, int precision) {
        std::stringstream ss;
        ss.precision(precision);
        ss << value / unit << " " << unitStr;
        return boxEquals(indent, keyWidth, key, ss.str(),
                         equalSign, nspaces, precision);
    }
    ...
    std::string warning(const std::string &what,
        ...
        // box
        std::stringstream ss;
        ss << boxTitle(head, fmt::gBoxWidth, fill);
        ss << src << msg << time;
        ss << boxBaseline(fmt::gBoxWidth, fill) << "\n\n";
        return ss.str();
    }
