...
setElement(const std::shared_ptr<const SolidElement> &element,
    ...
    // base
    Station::setElement(weights, mElement->getNu() + 1);
}
...
setInGroup(int dumpIntv, const channel::solid::ChannelOptions &chops) {
    ...
    // workspace
    expandWorkspaceRecord(mElement->getNu() + 1, chops);
...
record(int bufferLine, const channel::solid::ChannelOptions &chops) {
    ...
    if (chops.mNeedBufferG) {
        ...
        maskPoint<9>(sGXN9, mElement->getPointNu_1(), nu_1);
    ...
    if (chops.mNeedBufferE) {
        ...
        maskPoint<6>(sEXN6, mElement->getPointNu_1(), nu_1);
    ...
    if (chops.mNeedBufferR) {
        ...
        maskPoint<3>(sRXN3, mElement->getPointNu_1(), nu_1);
    ...
    if (chops.mNeedBufferS) {
        ...
        maskPoint<6>(sSXN6, mElement->getPointNu_1(), nu_1);
...
/////////////////////////// process ///////////////////////////
// process
void StationSolid::process(int bufferLine,
    ...
    for (int ich = 0; ich < chops.mStdChannels.size(); ich++) {
        ...
        if (ftype == channel::solid::FieldType::Displ) {
            ...
            throw std::runtime_error("StationSolid::process || "
                                     "Unknown field type.");
        }
    }
}
