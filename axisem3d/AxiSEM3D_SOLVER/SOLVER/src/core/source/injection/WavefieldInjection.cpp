...
initializeElements(std::vector<int> &interiorQuadTags,
    ...
    const std::vector<int> &sidePoints = se_tools::getPointsOnAllSides();
    ...
    for (const auto &elem: domain.getSolidElements()) {
        // interior
        if (vector_tools::foundSU(interiorQuadTags, elem->getQuadTag())) {
            inSolidElems.push_back(elem);
            continue; // cannot be both exterior and interior
        }
        // exterior
        if (vector_tools::foundSU(exteriorQuadTags, elem->getQuadTag())) {
...
void WavefieldInjection::
initializeSTFs(const std::string &ncFileNameS,
	       const std::string &ncFileNameF,
    ...
    // file
    mReaderS = std::make_shared<NetCDF_Reader>();
    mReaderF = std::make_shared<NetCDF_Reader>();
#ifdef _USE_PARALLEL_NETCDF
    mReader->openParallel(ncFileName);
#else
    mReaderS->open(ncFileNameS);
    mReaderF->open(ncFileNameF);
#endif
    
    // solid
    for (const auto &elem: mInteriorSolidElements) {
        elem->setSTF(aligned, bufferSize, mReaderS);
    }
    
    // fluid
    for (const auto &elem: mInteriorFluidElements) {
        elem->setSTF(aligned, bufferSize, mReaderF);
    }
