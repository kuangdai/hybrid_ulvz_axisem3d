...
PRT *Relabelling::createPRT(bool elem1D) const {
    ...
    const RDMatXN &XJ = getStiffJacobian();
    const RDMatXN4 &X = getStiffX();
    if (elem1D) {
        std::array<RMatPP, 5> xstruct;
        ...
        RDRowN xi_flat = XJ.block(0, 0, 1, nPE);
        RDMatPP xi_strct;
        XMath::structuredUseFirstRow(xi_flat, xi_strct);
        xstruct[4] = xi_strct.cast<Real>();
        
        return new PRT_1D(xstruct);
    } else {
        return new PRT_3D(X.cast<Real>(), XJ.cast<Real>());
    }
}
