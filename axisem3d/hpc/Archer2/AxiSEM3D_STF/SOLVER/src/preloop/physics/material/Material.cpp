...
void Material::addVolumetric3D(const std::vector<Volumetric3D *> &m3D, 
    ...
    // std::cout << "ULVZ: " << rElemCenter<<" "<< thetaElemCenter<< std::endl;
    // hardcode ulvz
    double ru_min = 3480e3;
    double ru_mid = ru_min + 2e3;
    double ru_max = ru_min + 20e3;
    double tu_max = 7.5 * degree;
    if (thetaElemCenter < tu_max) {
        if (ru_min < rElemCenter && rElemCenter < ru_mid) {
            mVsv1D *= (1. - .4);
            mVsh1D *= (1. - .4);
            mVpv1D *= (1. - .2);
            mVph1D *= (1. - .2);
            mRho1D *= (1. + .1);
            std::cout << "ULVZ: " << mMyQuad->getQuadTag() << " dvs=-40%" << std::endl;
        } else if (ru_mid < rElemCenter && rElemCenter < ru_max) {
            mVsv1D *= (1. - .2);
            mVsh1D *= (1. - .2);
            mVpv1D *= (1. - .1);
            mVph1D *= (1. - .1);
            mRho1D *= (1. + .05);
            std::cout << "ULVZ: " << mMyQuad->getQuadTag() << " dvs=-20%" << std::endl;
        }
    }
