...
namespace mpi {
    ...
    void bcast(std::vector<std::string> &vecStr, int src) {
        ...
        if (rank() == src) {
            lengths = std::vector<int>((int)vecStr.size(), -1);
            for (int istr = 0; istr < vecStr.size(); istr++) {
                flattened += vecStr[istr];
                lengths[istr] = (int)vecStr[istr].length();
            }
        }
        ...
        // cast back
        if (rank() != src) {
            vecStr = std::vector<std::string>((int)lengths.size(), "");
            int pos = 0;
            for (int istr = 0; istr < lengths.size(); istr++) {
                vecStr[istr] = flattened.substr(pos, lengths[istr]);
                pos += lengths[istr];
            }
        }
    }
    ...
    void gather(const std::string &str,
        ...
        // std::vector<char> to string
        if (dest < 0 || dest == rank()) {
            vecStr = std::vector<std::string>(nproc(), "");
    ...
    void gather(const std::vector<std::string> &vecStr,
        ...
        std::vector<int> localFlattenedLength((int)vecStr.size(), 0);
        for (int istr = 0; istr < vecStr.size(); istr++) {
