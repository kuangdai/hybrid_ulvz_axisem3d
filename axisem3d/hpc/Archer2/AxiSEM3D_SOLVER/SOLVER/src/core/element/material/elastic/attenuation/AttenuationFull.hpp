...
public:
    // 1D constructor
    AttenuationFull(const eigen::DColX &alpha,
                    const eigen::DColX &betta,
                    const eigen::DColX &gamma,
                    const eigen::DMatPP_RM &dLambda,
    ...
    // stream constructor
    AttenuationFull(sbs::ifstream &ifs):
    Attenuation(ifs),
    ...
    void checkCompatibility(int nr, bool elemInFourier, bool elastic1D) {
        ...
        mMemVar_FR = std::vector<eigen::vec_ar6_CMatPP_RM>(nsls(), mDStress_FR);
        mMemVar_CD = std::vector<eigen::RMatXN6>(nsls(), mDStress_CD);
