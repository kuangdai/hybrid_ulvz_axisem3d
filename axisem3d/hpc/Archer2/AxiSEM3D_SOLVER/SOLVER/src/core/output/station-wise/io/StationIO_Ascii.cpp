...
void StationIO_Ascii::initialize(const std::string &groupName,
    ...
    if (mpi::rank() == mRankWithMaxNumStations) {
        createFileStream(group_dir + "/time_points", precisionTime);
    }
    
    // data
    if (mStationCentric) {
        for (const std::string &stKey: stKeys) {
            createFileStream(group_dir + "/" + stKey, precisionWave);
        }
        // channel_order.info
        if (mpi::rank() == mRankWithMaxNumStations) {
            std::ofstream fout(group_dir + "/channel_order.info");
            if (!fout) {
                throw std::runtime_error("StationIO_Ascii::initialize || "
                                         "Error creating channel index file: ||"
                                         + group_dir + "/channel_order.info");
            }
        ...
        const std::string &rank_dir = group_dir + "/dir_rank" + mpi::strRank();
        io::mkdir(rank_dir);
        for (const std::string &channel: channels) {
            createFileStream(rank_dir + "/" + channel, precisionWave);
        }
        // station_order.info
        std::ofstream fout(rank_dir + "/station_order.info");
        if (!fout) {
            throw std::runtime_error("StationIO_Ascii::initialize || "
                                     "Error creating station index file: ||"
                                     + rank_dir + "/station_order.info");
        }
        for (const std::string &stKey: stKeys) {
            fout << stKey << "\n";
        }
