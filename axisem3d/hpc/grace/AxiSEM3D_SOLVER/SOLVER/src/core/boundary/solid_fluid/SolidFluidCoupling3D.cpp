...
// stream constructor
SolidFluidCoupling3D::
SolidFluidCoupling3D(sbs::ifstream &ifs, const Domain &domain):
SolidFluidCoupling(ifs, domain),
mNormal_UnassembledMPI(ifs.get<eigen::RMatX3>()),
mNormal_AssembledMPI_InvMassFluid(ifs.get<eigen::RMatX3>()) {
    checkCompatibility(mSolidPoint->getNr());
}
...
void SolidFluidCoupling3D::coupleSolidToFluid(const eigen::CMatX3 &solidDispl,
    ...
    // FFT: Fouier => cardinal
    fftw::gFFT_3.computeC2R(solidDispl, sSolidR, nr);
    ...
    fftw::gFFT_1.computeR2C(sFluidR, sFluidC, nr);
...
void SolidFluidCoupling3D::coupleFluidToSolid(const eigen::CColX &fluidStiff,
    ...
    // FFT: Fouier => cardinal
    fftw::gFFT_1.computeC2R(fluidStiff, sFluidR, nr);
    ...
    // FFT: cardinal => Fourier
    fftw::gFFT_3.computeR2C(sSolidR, sSolidC, nr);
