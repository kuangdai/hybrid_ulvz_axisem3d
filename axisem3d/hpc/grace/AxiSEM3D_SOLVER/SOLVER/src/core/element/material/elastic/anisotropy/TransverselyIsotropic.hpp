...
public:
    ...
    // stream constructor
    TransverselyIsotropic(sbs::ifstream &ifs):
    Elastic(ifs),
    mA(ifs), mC(ifs), mF(ifs), mL(ifs), mN(ifs)
#ifndef _SAVE_MEMORY
    , mMinusN2(ifs)
#endif
    {
    ...
    void checkCompatibility(int nr, bool elemInFourier) const {
        ...
        // parameters
        mA.checkCompatibility(m1D, nr, elemInFourier, "mA",
                              "TransverselyIsotropic");
        
        // workspace
#ifdef _SAVE_MEMORY
        sMinusN2.expandSize(m1D, nr);
#endif
    }
