...
public:
    ...
    // stream constructor
    PRT(sbs::ifstream &ifs): m1D(ifs.get<bool>()),
    mX0(ifs), mX1(ifs), mX2(ifs), mX3(ifs), mXJ(ifs) {
        // noting
    }
#else
    // 1D constructor
    ...
    // stream constructor
    PRT(sbs::ifstream &ifs): m1D(ifs.get<bool>()),
    mX0(ifs), mX1(ifs), mX2(ifs), mX3(ifs),
    mX0_J(ifs),
    mX1_J(ifs),
    mX2_J(ifs),
    mX3_J(ifs) {
    ...
    void checkCompatibility(int nr, bool elemInFourier) const {
        ...
        // workspace
#ifdef _SAVE_MEMORY
        sX0_J.expandSize(m1D, nr);
        sX1_J.expandSize(m1D, nr);
        sX2_J.expandSize(m1D, nr);
        sX3_J.expandSize(m1D, nr);
#endif
    }
