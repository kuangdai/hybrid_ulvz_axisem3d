...
public:
    ...
    // stream constructor
    GradientQuadrature(sbs::ifstream &ifs):
    mDsDxii(ifs.get<RMatPP_RM>()), mDsDeta(ifs.get<RMatPP_RM>()),
    mDzDxii(ifs.get<RMatPP_RM>()), mDzDeta(ifs.get<RMatPP_RM>()),
    m1overS(ifs.get<RMatPP_RM>()), mAxial(ifs.get<bool>()),
    mIntegralFactor(ifs.get<RMatPP_RM>()) {
        // nothing
    }
#else
    // parameter constructor
    ...
    mDzDxii(ifs.get<RMatPP_RM>()), mDzDeta(ifs.get<RMatPP_RM>()),
    m1overS(ifs.get<RMatPP_RM>()), mAxial(ifs.get<bool>()),
    mDsDxii_IF(ifs.get<RMatPP_RM>()),
    mDsDeta_IF(ifs.get<RMatPP_RM>()),
    mDzDxii_IF(ifs.get<RMatPP_RM>()),
    mDzDeta_IF(ifs.get<RMatPP_RM>()),
    m1overS_IF(ifs.get<RMatPP_RM>()) {
        // nothing
    }
#endif
...
private:
    ...
    void computeGrad_dSdZ(int nu_1, const VecArPP_F &F, int dimF,
        ...
        // loop over alpha
        for (int alpha = 0; alpha < nu_1; alpha++) {
            sX = GT_xii * F[alpha][dimF];
            sY = F[alpha][dimF] * sG_GLL;
    ...
    void computeGrad_1overS_setTo(int nu_1, const VecArPP_A &A, int dimA,
        ...
        for (int alpha = 0; alpha < nu_1; alpha++) {
            ...
            // axial
            if (mAxial) {
                sX.row(0) = GT_xii.row(0) * A[alpha][dimA];
                sY.row(0) = A[alpha][dimA].row(0) * sG_GLL;
    ...
    const {
        ...
        for (int alpha = 0; alpha < nu_1; alpha++) {
            ...
            F[alpha][dimF] = G_xii * sX + sY * sGT_GLL;
        }
    }
    ...
    void computeQuad_1overS_addTo(int nu_1, const VecArPP_A &A, int dimA,
        ...
        for (int alpha = 0; alpha < nu_1; alpha++) {
            ...
            if (mAxial) {
                ...
                AoS[alpha][dimAoS] -= G_xii.col(0) * sX.row(0);
                AoS[alpha][dimAoS].row(0) -= sY.row(0) * sGT_GLL;
            }
        }
    }
