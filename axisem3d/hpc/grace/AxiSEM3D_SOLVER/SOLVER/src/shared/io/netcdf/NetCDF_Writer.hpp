...
public:
    ...
    ////////////////// write variable //////////////////
    // write data to variable chunk (no id and datatype check)
    ...
    // write data to variable chunk (with id and datatype check)
    template <class Container>
    void writeVariable(const std::string &vname, const Container &val,
                       const std::vector<numerical::Int> &starts,
                       const std::vector<numerical::Int> &counts) const {
        int varid = netcdf::getVariableID(mPWD, vname, mFileName, val);
