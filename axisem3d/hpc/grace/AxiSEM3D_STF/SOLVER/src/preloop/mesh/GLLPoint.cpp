...
int GLLPoint::release(Domain &domain, int meshTag) {
    // solid fluid
    ...
    if (isSolid && isFluid) {
        ...
        mSolidDomainTag = domain.addSolidPoint(solid);   
        mFluidDomainTag = domain.addFluidPoint(fluid);   
        solid->mMeshTag = meshTag;
        fluid->mMeshTag = meshTag;
        
        
        SolidFluidPoint *sfpoint = new SolidFluidPoint(solid, fluid, couple);
        sfpoint->mSolidTag = mSolidDomainTag;
        sfpoint->mFluidTag = mFluidDomainTag;
        sfpoint->mMeshTag = meshTag;
