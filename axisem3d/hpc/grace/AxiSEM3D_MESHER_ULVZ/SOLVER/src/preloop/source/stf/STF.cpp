...
void STF::buildInparam(STF *&stf, const Parameters &par, double dt, int verbose) {
    ...
    double record_interval = par.getValue<int>("OUT_STATIONS_RECORD_INTERVAL");
    std::string mstf = par.getValue<std::string>("SOURCE_TIME_FUNCTION");
    if (boost::iequals(mstf,"erf")) {
        // Heaviside
        stf = new ErfSTF(dt, duration, record_interval, hdur, decay);
    } else if (boost::iequals(mstf,"gauss")) {
        // Gaussian
        stf = new GaussSTF(dt, duration, record_interval, hdur, decay);
    } else if (boost::iequals(mstf,"ricker")){
        // Ricker
        stf = new RickerSTF(dt, duration, record_interval, hdur, decay);
    } else {
