...
void Material::addVolumetric3D(const std::vector<Volumetric3D *> &m3D, 
    ...
    double thetaElemCenter = mMyQuad->computeCenterTheta();
        ...
        for (int jpol = 0; jpol <= nPol; jpol++) {
            ...
            for (int alpha = 0; alpha < Nr; alpha++) {
                ...
                for (const auto &model: m3D) {
                    ...
                    if (!model->get3dProperties(r, t, p, rElemCenter, thetaElemCenter,
    ...
    static int with_ulvz = 0;
    static std::vector<double> r, dvs, dvp, drho;
    static double tmin = 0, tmax = 0;
    
    if (!initialized) {
        if (XMPI::root()) {
            std::ifstream ifs;
            ifs.open("./input/ULVZ.txt");
            if (!ifs) {
                throw std::runtime_error("ULVZ.txt does not exist.");
            }
            ifs >> with_ulvz;
            if (with_ulvz) {
                int n_layer = 0;
        ...
        if (XMPI::root()) {
            std::cout << "ULVZ:\n";
            std::cout << "Activated: " << with_ulvz << "\n";
            if (with_ulvz) {
                std::cout << "r dvs dvp drho\n";
                for (int i =0; i<r.size();i++) {
                    std::cout << r[i] << " " << dvs[i] << " " << dvp[i] << " " << drho[i] << "\n";
                }
                std::cout << "theta (min, max) = (" << tmin << ", " << tmax << ")\n";
            }
        }
        initialized = true;
    }
    ...
    if (tmin < thetaElemCenter && thetaElemCenter < tmax 
        && r.front() < rElemCenter && rElemCenter < r.back() && with_ulvz) {
        // for mass
	    prepare3D();
            ...
            if (rElemCenter < r[i]) {
                for (int ipol = 0; ipol <= nPol; ipol++) {
                    for (int jpol = 0; jpol <= nPol; jpol++) {
                        ...
                        int ipnt = ipol * nPntEdge + jpol;
                        for (int alpha = 0; alpha < Nr; alpha++) {
                            double r_ = rtp(alpha, 0);
                            double dvs_ = (dvs[i] - dvs[i-1]) / (r[i]-r[i-1]) * (r_-r[i-1]) + dvs[i-1];
                            double dvp_ = (dvp[i] - dvp[i-1]) / (r[i]-r[i-1]) * (r_-r[i-1]) + dvp[i-1];
                            double drho_ = (drho[i] - drho[i-1]) / (r[i]-r[i-1]) * (r_-r[i-1]) + drho[i-1];
                            mVsv3D(alpha, ipnt) *= (1. + dvs_);
                            mVsh3D(alpha, ipnt) *= (1. + dvs_);
                            mVpv3D(alpha, ipnt) *= (1. + dvp_);
                            mVph3D(alpha, ipnt) *= (1. + dvp_);
                            mRho3D(alpha, ipnt) *= (1. + drho_);
                        }
                    }
                }
                break;
...
Elastic *Material::createElastic(bool elem1D, const AttBuilder *attBuild) const {
    ...
    for (int ipnt = 0; ipnt < nPntElem; ipnt++) {
        // A C L N
        A.col(ipnt).array() *= vph3D.col(ipnt).array().pow(2.);
        C.col(ipnt).array() *= vpv3D.col(ipnt).array().pow(2.);
        L.col(ipnt).array() *= vsv3D.col(ipnt).array().pow(2.);
        N.col(ipnt).array() *= vsh3D.col(ipnt).array().pow(2.);
    }
    // F
    F = eta3D.schur(A - 2. * L);
