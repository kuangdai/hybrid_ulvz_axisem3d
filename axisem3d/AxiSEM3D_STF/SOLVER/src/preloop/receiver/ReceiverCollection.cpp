...
mSaveSurfaceAtRadius(saveSurfRadius), mSaveSurfaceFromUpper(saveSurfUpper) {
    ...
    std::vector<int> forceFluid;
    std::vector<int> forceSolid;
    if (!boost::iequals(fileRec, "none")) {
                ...
                try {
                    ...
                    if (npar > 6) {
                        for (int ipar = 6; ipar < npar; ipar++) {
                            ...
                            if (boost::iequals(strs[ipar], "force_fluid")) {
                                force_fluid = 1;
                            }
                            if (boost::iequals(strs[ipar], "force_solid")) {
                                force_solid = 1;
                            }
                        }
                    }
                    ...
                    forceFluid.push_back(force_fluid);
                    forceSolid.push_back(force_solid);
                } catch(std::exception) {
        ...
        XMPI::bcast(dumpCurl);
        XMPI::bcast(forceFluid);
        XMPI::bcast(forceSolid);
    }
    ...
    for (int i = 0; i < name.size(); i++) {
        ...
        mReceivers.push_back(new Receiver(name[i], network[i], 
            theta[i], phi[i], geographic, depth[i], 
            (bool)dumpStrain[i], (bool)dumpCurl[i], 
            (bool)forceFluid[i], (bool)forceSolid[i],
