...
int Quad::releaseSolid(Domain &domain, const IMatPP &myPointTags, const AttBuilder *attBuild) const {
    ...
    SolidElement *elem = new SolidElement(grad, prt, points, elas, mQuadTag);
    domain.addSolidElement(elem);
...
int Quad::releaseFluid(Domain &domain, const IMatPP &myPointTags) const {
    ...
    FluidElement *elem = new FluidElement(grad, prt, points, acous, mQuadTag);
    domain.addFluidElement(elem);
    return domain.addElement(elem);
}
...
double Quad::computeCenterTheta() const {
    // xi = eta = 0
    const RDCol2 &sz = mapping(RDCol2::Zero());
    return acos(sz(1) / std::max(sz.norm(), tinyDouble));
}
...
Gradient *Quad::createGraident() const {
    ...
    return new Gradient(dsdxii, dsdeta, dzdxii, dzdeta, inv_s, mIsAxial, mIntegralFactor);  
}
